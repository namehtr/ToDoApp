...
from models import (get_user_by_email, add_user, get_all_todos_by_email, add_todo_for_email,
                   get_todo_by_id, add_subtask_to_todo, update_todo_status, update_subtask_status)
...
def register():
    ...
    try:
        ...
        app.logger.error('Database error occurred during registration!', exc_info=True)
...
def login():
    ...
    if request.method == 'POST':
        try:
            ...
            app.logger.error('Database error occurred during login!', exc_info=True)
            flash('Database error occurred!', 'danger')
...
def add_subtask():
    ...
    try:
        ...
        app.logger.error('Database error occurred while adding subtask!', exc_info=True)
        flash('Database error occurred!', 'danger')
    return redirect(url_for('home'))
